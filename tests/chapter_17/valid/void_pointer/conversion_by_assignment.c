...
int main(void) {
  ...
  *int_ptr = -1; // set all bytes to 1
  ...
  if (dbl_ptr != four_bytes || complicated_ptr != four_bytes ||
      long_ptr != four_bytes) {
    return 3;
  }
  ...
  if (some_other_ptr) { // null pointer is always 0 no matter what type it
                        // points to
    return 8;
  }
  ...
  long *long_ptr_array[3] = {// convert three pointers from (void *) to (long *)
                             malloc(sizeof(long)), malloc(sizeof(long)),
                             malloc(sizeof(long))};
  ...
  *long_ptr_array[2] = 300l;
  ...
  /* one last test case: implicitly convert pointers to void * when we pass them
  ...
  if (memcmp(arr1, arr3, sizeof arr2) !=
      -1) { // arr1 should compare less than arr3, since 3 < 4
    return 11;
  }
  return 0;
}
