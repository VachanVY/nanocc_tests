...
int main(void) {
  ...
  if (!pass_unions_and_structs(1, 2, one_gp_struct, 4.0, two_xmm, one_gp, 100,
                               120, 130)) {
    return 1;
  }

  // out of general-purpose regs, pass the union in memory
  if (!pass_gp_union_in_memory(two_xmm, one_gp_struct, -1, -2, -3, -4, -5, -6,
                               one_gp)) {
    return 2;
  }
  ...
  if (!pass_xmm_union_in_memory(1.0, 2.0, two_xmm, two_xmm, 3.0, 4.0,
                                two_xmm_2)) {
    return 3;
  }
  ...
  // same idea but w/ union passed in a mix of registers - we have a free XMM
  // reg but not a free general-purpose reg
  union gp_and_xmm mixed_regs = {{0, 150.5}};
  if (!pass_mixed_reg_in_memory(101.2, 102.3, 103.4, 104.5, 75, 76, 77, 78, 79,
                                80, mixed_regs)) {
    return 6;
  }
  ...
  if (!pass_uneven_union_in_memory(1100, 2200, 3300, 4400, 5500, mixed_regs,
                                   one_gp, uneven)) {
    return 7;
  }
