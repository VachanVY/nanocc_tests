/* Test access to nested union members through dot, arrow, and subscript
 * operators */
...
int test_auto_arrow(void) {
  ...
  if (outer_ptr->u_ptr->c != -10 || outer_ptr->u_ptr->i != -10 ||
      outer_ptr->u_ptr->l != -10) {
    return 0; // fail
  }

  // read through members of uc_arr
  if (outer_ptr->u_ptr->uc_arr[0] != 246 ||
      outer_ptr->u_ptr->uc_arr[1] != 255 ||
...
int test_array_of_union_pointers(void) {
  ...
  if (ptr_arr[0]->u_ptr->l != 0 || ptr_arr[1]->u_ptr->l != 1 ||
      ptr_arr[2]->u_ptr->l != 2) {
    return 0; // fail
  }
