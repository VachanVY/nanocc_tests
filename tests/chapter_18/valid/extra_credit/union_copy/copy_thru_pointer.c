// Test copying whole structs/unions through pointers (incl. to/from array
// members)
...
int test_copy_to_pointer(void) {
  ...
  *x = y;
  ...
  // validate
  if (x->l != -20 || x->i != -20 || x->uc_arr[0] != 236 ||
      x->uc_arr[1] != 255 || x->uc_arr[2] != 255) {
    return 0; // fail
  }
...
int test_copy_from_pointer(void) {
  ...
  // validate
  if (another_union.s.l != 8223372036854775807l || another_union.s.d != 20e3 ||
      another_union.s.u != 2147483650u) {
    return 0; // fail
  }
...
int test_copy_array_members(void) {
  ...
  // validate
  if (strcmp(union_array[0].arr, "foobar") ||
      strcmp(union_array[1].arr, "hello") ||
      strcmp(union_array[2].arr, "blahblah")) {
    return 0; // fail
  }
