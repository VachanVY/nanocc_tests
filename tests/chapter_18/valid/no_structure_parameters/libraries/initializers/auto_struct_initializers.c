...
int validate_full_initialization(struct s* ptr) {
    if (strcmp(ptr->one_msg, "I'm a struct!") || ptr->two_arr[0] != 's' || ptr->two_arr[1] != 'u' ||
        ptr->two_arr[2] != 'p' || ptr->three_self_ptr != ptr || ptr->four_d != 2e12 ||
        *ptr->five_d_ptr != 2e12) {
        return 0;
    }
...
int validate_partial_initialization(struct s* ptr, char* expected_msg) {
    if (ptr->one_msg != expected_msg || ptr->two_arr[0] != 'a' || ptr->two_arr[1] != 'b') {
        return 0;
    }
...
int validate_converted(struct s* ptr) {
    if (!ptr->one_msg || // just validate that this pointer isn't null
        ptr->two_arr[0] != 220 || ptr->two_arr[1] != 232 || ptr->two_arr[2] != 224 ||
        ptr->three_self_ptr || ptr->four_d != 2999.0 || *ptr->five_d_ptr != 0.0) {
        return 0;
    }
...
int validate_two_structs(struct s* ptr1, struct s* ptr2) {
    ...
    if (strcmp(ptr2->one_msg, "Yet another string") ||
        ...
        ptr2->three_self_ptr != ptr1 || // ptr2->three_self_ptr is ptr1, not to itself
