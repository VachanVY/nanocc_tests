 ...
 * so we know which objects are right next to the one we copy to. If this
 * assumption doesn't hold, clobbers may overwrite stack padding instead of data
 * that we validate, and go undetected).
 */
...
// helpers to validate other stuff on stack without generating any other
// temporary variables
