// Test that we can constant-fold operations on NaN (and propagate NaN like
// other values); similar to chapter_13/valid/extra_credit/nan.c.
...
int target(void) {
  ...
  // (we detect that a value is nan by making sure it doesn't compare equal to
  // itself)
  nan = nan * 4;
  if (nan == nan) {
    return 5;
