/* Test copy prop/constant folding of compound bitwise assignment with
 * non-integer types and type conversions
 * TODO: use templates for duplicate code between here and earlier chapters
 * instead of copy-paste (ditto for other extra-credit constant-folding tests
 * too!)
 */
...
int target_chars(void) {
  ...
  c4 <<=
      7u; // this wraps around to -128; well-defined b/c of integer promotions
...
int target_long_bitwise(void) {
  ...
  // if lval is int, convert to common type, perform operation, then convert
  // back
  i = -2147483648l; // 0x8000_0000
  // check result and side effect
  // 1. sign extend 0x8000_0000 to 0xffff_ffff_8000_0000
  // 2. calculate 0xffff_ffff_8000_0000 | 0x00ff_00ff_00ff_00ff =
  // 0xffff_ffff_80ff_00ff
...
int target_unsigned_bitwise(void) {
  ...
  ul |= 4294967040u; // 0xffff_ff00 - make sure we zero-extend this to unsigned
                     // long
