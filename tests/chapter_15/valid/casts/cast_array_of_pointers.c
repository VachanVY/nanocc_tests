...
int main(void) {
    ...
    // Note #1: dereferencing other_ptr would violate strict aliasing, but casting/comparing it is
    // okay. Note #2: casting between pointer types is undefined if the pointer is misaligned for
    // the new type; this specific case is safe because we're casting from pointer-to-pointer to
    // pointer-to-long, and pointers and longs are both eight-byte aligned
