...
int check_null_ptr_cast(void) {
  /* You can cast a null pointer to any pointer type and the result is still a
   * null pointer */
  static long *long_ptr =
      0; // make this static so we don't optimize away this function
...
int check_round_trip(void) {
   ...
   * we're only dealing with pointers to 8 byte-aligned types so it's not a
   * problem
   */
  long l = -1;
  long *long_ptr = &l;
