...
int target(int arg) {
  ...
  if (flag) {
    ...
    // It creates conflict between a-m, which will go away as we perform
    // coalescing. the same value
    a = arg;
    b = arg;
    c = arg;
    d = arg;
    ...
    // This branch creates conflicts between a-m, which will go away as we
    // perform coalescing.
    a = glob * 2; // 10
    b = a;
    c = a;
