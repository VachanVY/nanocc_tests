 ...
 * with all pseudos in both of them. to_spill has high degree but also a
 * slightly higher spill cost than the other pseudos. If we use spill_cost /
 * degree as our spill metric, we'll only spill to_spill and nothing else. If we
 * only use spill cost, and not degree, we'll spill two pseudos, one from each
 * clique, resulting in more memory accesses overall.
 */
