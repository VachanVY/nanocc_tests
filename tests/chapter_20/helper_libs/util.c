...
int check_5_ints(int a, int b, int c, int d, int e, int start) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (args[i] != expected) {
            printf("Expected argument %d to have value %d, actual value was %d\n", i, start + i,
                   args[i]);
            exit(-1);
        }
    }
...
int check_12_longs(long a, long b, long c, long d, long e, long f, long g, long h, long i, long j,
    ...
    for (int i = 0; i < 12; i++) {
        ...
        if (args[i] != expected) {
            printf("Expected argument %d to have value %ld, actual value was "
                   "%ld\n",
                   i, start + i, args[i]);
            exit(-1);
        }
...
int check_14_doubles(double a, double b, double c, double d, double e, double f, double g, double h,
    ...
    for (int i = 0; i < 14; i++) {
        ...
        if (args[i] != expected) {
            printf("Expected argument %d to have value %f, actual value was %f\n", i, start + i,
                   args[i]);
            exit(-1);
        }
    }
...
double dbl_id(double x) { return x; }

long long_id(long l) { return l; }

unsigned unsigned_id(unsigned u) { return u; }

unsigned char uchar_id(unsigned char uc) { return uc; }