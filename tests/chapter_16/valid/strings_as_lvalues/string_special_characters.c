...
int main(void) {
  ...
  I used octal codes for all special characters but always pad them out to three
  digits, so e.g. " 123" was escaped as "\40123", which was interpreted as the
  invalid octal code \401 followed by "23". The correct way to escape this is
  \040123, since the assembler recognizes a backslash followed by up to three
  octal digits as an escape sequence.
  */
  puts("Testing, 123.");
  /* Now test that we don't add a leading zero to an escape sequence that's
  already three digits.
  "^@" should be escaped as "\136\100". If we escape it as "\0136\0100" the
  assembler will interpret it as \013 6 \010 0
  */
  puts("^@1 _\\]");
